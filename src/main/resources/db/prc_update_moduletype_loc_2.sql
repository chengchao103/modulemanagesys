DELIMITER $$
DROP PROCEDURE IF EXISTS prc_update_moduletype_loc_2$$
CREATE PROCEDURE prc_update_moduletype_loc_2()
  BEGIN
    /*
    根据设备型号统计设备分布
    ...
    */
    ...
    FOR SELECT
          ...
          count(sn) AS module_count
        FROM tm_main_modules_tmp2
        GROUP BY customer_name, module_type, province
    ...
    REPEAT
      ...
      INTO p_module_type, p_province, p_module_count;
      BEGIN
        SELECT count(*)
        INTO p_num
        FROM ts_sys_statistics_analysis
        WHERE module_type = p_module_type AND province = p_province AND city = -1 AND
    ...
    REPEAT
      ...
      INTO p_customer_name, p_module_type, p_province, p_module_count;
      BEGIN
        SELECT count(*)
        INTO p_num
        FROM ts_sys_statistics_analysis
