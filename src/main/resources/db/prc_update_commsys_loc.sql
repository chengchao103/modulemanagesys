DELIMITER $$
DROP PROCEDURE IF EXISTS prc_update_commsys_loc$$
CREATE PROCEDURE prc_update_commsys_loc()
  BEGIN
    /*
    根据通讯制式统计设备分布
    ...
    */
    ...
    FOR SELECT
        ...
        GROUP BY comm_sys, province, city
        ORDER BY comm_sys;
    ...
    FOR SELECT
          ...
          count(sn) AS module_count
        FROM tm_main_modules_tmp1
        GROUP BY customer_name, comm_sys, province, city
    ...
    CREATE INDEX tm_main_modules_tmp1_idx4 ON tm_main_modules_tmp1 (city);


    OPEN commsys_loc;
    REPEAT
      FETCH commsys_loc
      INTO p_comm_sys, p_province, p_city, p_module_count;
      BEGIN
        SELECT count(*)
        INTO p_num
        FROM ts_sys_statistics_analysis
        WHERE comm_sys = p_comm_sys AND province = p_province AND city = p_city AND
    ...
    REPEAT
      ...
      INTO p_customer_name, p_comm_sys, p_province, p_city, p_module_count;
      BEGIN
        SELECT count(*)
        INTO p_num
        FROM ts_sys_statistics_analysis
